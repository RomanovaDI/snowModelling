...
class simulation():
	...
	def __init__(self):
		...
		O.engines=[
			...
			InsertionSortCollider([Bo1_Sphere_Aabb(),Bo1_Facet_Aabb(), Bo1_Cylinder_Aabb()], allowBiggerThanPeriod=True),
			...
			#PyRunner(command='sim.printDt()', iterPeriod= 1, label='outputMessage'),
			#PyRunner(command='sim.setDt(1e-4)', iterPeriod= 1, label='outputMessage'),
			...
			#PyRunner(command='sim.printState(9)', iterPeriod= 1, label='outputMessage')
		]
